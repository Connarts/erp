                        ...
                        <!-- <div class="display-4 pb-3">
                          ...
                          <form class="" name="signup" role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control" name="Rbrandname" aria-describedby="brand name" placeholder="Enter brand name">
                            </div>
                            <div class="form-group">
                              <label for="Remail">Email address</label>
                              <input type="email" class="form-control" name="Remail" aria-describedby="email" placeholder="Enter email">
                              <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
                            </div>
                            <div class="form-group">
                              <label for="Rpassword">Password</label>
                              <input type="password" class="form-control" name="Rpassword" aria-describedby="password" placeholder="Password">
                            </div>
...
<?php
  ...
  if ( (isset($_POST['submit'])) && (!empty($_POST['email'])) && (!empty($_POST['password'])) ) {
    ...
    $u = htmlspecialchars($_POST["email"]); $p = htmlspecialchars($_POST["password"]);

    if (mysqli_query($con, "SELECT * FROM clients WHERE password = '$p' AND email = '$u' ")) {
...
if ( (isset($_POST['Rsubmit'])) && (!empty($_POST['Remail'])) && (!empty($_POST['Rpassword'])) && (!empty($_POST['Rbrandname']))  ) {
  ...
  $Re = htmlspecialchars($_POST["Remail"]); $Rb = htmlspecialchars($_POST["Rbrandname"]);
  $Rp = htmlspecialchars($_POST["Rpassword"]);

  if (mysqli_query($con, "SELECT email, password FROM clients WHERE password = '$Rp' AND email = '$Re' ") && mysqli_affected_rows($con) == 0 ) {
    # code...if nobody has taken those details then register them
      mysqli_query($con, "INSERT INTO clients (email, password, brandname) VALUES ('$Re', '$Rp', '$Rb')");
